# hpc4.cpp — Line-by-line explanation

This file documents each line of `hpc4.cpp` without modifying the original source.
...
- Program accepts `n` pairs (x,y), uses OpenMP reduction to compute sums required for ordinary least squares linear regression.
- Uses formulas for slope `m` and intercept `c` after parallel reduction.
- Execution time is measured using `omp_get_wtime()`.

Notes:
- Arrays `x` and `y` are fixed-size (100); ensure `n <= 100`.
